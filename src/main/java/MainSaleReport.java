...
public class MainSaleReport {
    ...
    private static void buildNestedTablesBody(PdfPTable outerTable, Font font) throws IOException, DocumentException {
        ...
        for (int i = 0; i < 30; i++ ){
            ...
            totalQuantity += i;
            bodyTable.addCell(createCell(String.valueOf(12000.0*i), font, 0.5f, 1, Element.ALIGN_RIGHT));
            bodyTable.addCell(createCell(String.valueOf(12000.0*i), font, 0.5f, 1, Element.ALIGN_RIGHT));
            bodyTable.addCell(createCell("วิภาวดี ม่อนคุต", font, 0.5f, 1, Element.ALIGN_LEFT));
            totalReceivedAmount += (12000.0*i);
        }
        bodyTable.addCell(createCell("รวม", font, 0.5f, 2, Element.ALIGN_CENTER));
        bodyTable.addCell(createCell(String.valueOf(totalQuantity), font, 0.5f, 1, Element.ALIGN_CENTER));
        bodyTable.addCell(createCell(String.valueOf(12000.0*totalQuantity), font, 0.5f, 1, Element.ALIGN_RIGHT));
        bodyTable.addCell(createCell(String.valueOf(totalReceivedAmount), font, 0.5f, 1, Element.ALIGN_RIGHT));
        bodyTable.addCell(createCell("บาท", font, 0.5f, 1, Element.ALIGN_LEFT));
